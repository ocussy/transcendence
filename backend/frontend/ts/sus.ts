class PoopEasterEgg {
  ...
  private isPoopMode = false;
  private targetSelector: string;

  constructor(targetSelector: string = "h1, .logo, [data-easter-egg]") {
  ...
  private triggerPoopAvalanche(): void {
    if (this.isPoopMode) return;
  ...
  private createFallingPoop(container: HTMLElement): void {
  ...
  private cleanupPoopAvalanche(): void {
    const container = document.getElementById("poop-container");
    if (container) container.remove();
    this.isPoopMode = false;
  }

  private addPoopAnimation(): void {
    if (document.getElementById("poop-animation")) return;
  ...
  private createSimpleAlert(message: string): void {
